...
- **Bounded clip cache.** A new **Cache limit (MB)** setting (default 512) caps the total; whole oldest clips are dropped first. The live buffer and the rolling backup are never evicted.
- **New panel body.** A top action bar (**Save all** / **Clear all**), a **Current buffer** section (**Save to disk** / **Cache clip** / **Reset buffer** plus the existing stats), and a **Cached clips** list. Each clip row shows its trigger, capture time, duration, size, and message count, expands into a per-topic message-count table, and has its own save and delete affordances. Delete and Clear all confirm inline. The clips heading reports how much of the cache is used and which OPFS access path is in use.
