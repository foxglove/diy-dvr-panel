...
- **Testable capture core.** The worker's capture logic moved into `captureEngine.ts`, which takes an injected storage backend, clock, and output sink, leaving `mcap.worker.ts` as a thin `postMessage` adapter. This mirrors the existing approach for `settings.ts`. Adds the repo's first test suite — Jest + ts-jest in a Node environment, with 64 unit tests covering the ring buffer, all clip triggers, auto-save rotation durability, session recovery and its resilience to transient storage errors, cache eviction priority, save-status severity, and the new settings; CI now runs them.
